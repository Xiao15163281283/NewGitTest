...
import mobilenet_v2,mobilenet_v2_ts1
...
class Trainer(object):
    ...
    def __init__(self, config, data_loader):
        ...
        for i in range(self.model_num):
            # build models
            checkpoint_0 = torch.load(r'./0.7031model/model_0.7031.pth.tar')
            model_0 = mobilenet_v2.MobileNetV2()
            model_0.load_state_dict(checkpoint_0, strict=False)
            ...
            optimizer_0 = optim.SGD(model_0.parameters(), lr=self.lr, momentum=self.momentum,
                                  weight_decay=self.weight_decay, nesterov=self.nesterov)

            self.optimizers.append(optimizer_0)

            # set learning rate decay
            scheduler = optim.lr_scheduler.StepLR(self.optimizers[i], step_size=10, gamma=self.gamma, last_epoch=-1)
            self.schedulers.append(scheduler)

            checkpoint_1 = torch.load(r'./remove_prune/pruned.pth.tar')
            model_1 = mobilenet_v2_ts1.MobileNetV2()
            ...
            # initialize optimizer and scheduler
            optimizer = optim.SGD(model_1.parameters(), lr=self.lr, momentum=self.momentum,
                                  weight_decay=self.weight_decay, nesterov=self.nesterov)
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            # check for improvement
                ...
                self.save_checkpoint(i,
                    ...
                    'best_prec': self.best_valid_accs[i],
                    }, is_best
                )
